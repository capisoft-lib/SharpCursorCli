...
/// <summary>
/// Resolves the Cursor Agent executable path on macOS: .env (CURSOR_AGENT_PATH) then environment variable then "agent" on PATH.
/// </summary>
internal static class AgentPathResolver
{
    /// <summary>Returns (fileName, baseArgs). On macOS baseArgs is always empty; run the agent binary/script directly.</summary>
